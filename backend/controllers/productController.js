...
exports.displayAllproduct = (req, res)=>{
      // console.log(req.query)
        db.query('SELECT * FROM product', (error, data)=> {
            ...
            // return res.send({status: "success", data : req.query})
                return res.send({status: "success", data})
        })
    // }else{
    //     return res.send({status: "Error", Error : "redirect user to login"}) 
...
exports.deleteproduct = (req, res)=>{
    const {id} = req.params;
    db.query('DELETE FROM product WHERE user_id = ? ', [id], (error, data1)=>{
      if(error){
